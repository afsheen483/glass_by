...
if (isset($_POST['submit'])) {
   ...
   // $session_userId;
    ...
    $current_password = ( md5(md5(sha1( $_POST['current_password'])).'Anomoz'));
   
    $new_password = ( md5(md5(sha1( $_POST['new_password'])).'Anomoz'));
    $confirm_password = ( md5(md5(sha1( $_POST['confirm_password'])).'Anomoz'));
    $query_selectedPost= "UPDATE `glassBuy_users` SET `password`='$new_password' WHERE `id` = '$session_userId'"; 
    
    $result_selectedPost = $con->query($query_selectedPost); 
    if ($result_selectedPost) {
        setFlash("error","Password Updated Successfully.","alert-success");
                ...
                <form method="post" action="<?php $_SERVER['PHP_SELF']?>" enctype="multipart/form-data">
                    <div class="form-wrap">
                        ...
                        <p class="woocommerce-form-row woocommerce-form-row--first form-row form-row-first">
                            <label for="type">Current Password<span class="required">*</span></label>
                            <input class="input-text" name="current_password"  id="type" required type="password">
                        </p>
                        <p class="woocommerce-form-row woocommerce-form-row--first form-row form-row-first">
                            <label for="type">New Password<span class="required">*</span></label>
                            <input class="input-text" name="new_password"  id="type" type="password" min="0" max="100" required>
                        </p>
                        <p class="woocommerce-form-row woocommerce-form-row--first form-row form-row-first">
                            <label for="type">Confirm Password<span class="required">*</span></label>
                            <input class="input-text" name="confirm_password"  id="type" type="password"  required>
                        </p>
                        ...
                        <div >
                            ...
                            <button class="btn-link" type="submit" style="margin-top:5%;margin-left: 20px;" name="submit">Submit</button>
